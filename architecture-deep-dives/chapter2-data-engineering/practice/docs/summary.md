...
以下方向在本次实战中已识别但未深入，留作后续线索：

- **不同 LLM 的 HyDE 精度对比** — 本实战使用 Qwen3-4B，可对比 Gemma 4 26B 等更大模型在 HyDE 生成质量与幻觉抑制上的差异
- **生产级图谱存储** — LightRAG 内置的文件存储适合原型，生产环境可替换为 Neo4j 或 NebulaGraph 以支持复杂图遍历
- **Schema 干预实验** — 对比不同 Prompt 策略对实体抽取精度的影响，建立"Schema 精细度 vs 检索质量"的量化关系
- **CI 级 Schema 一致性测试** — 自动检测实体类型体系在不同批次间的漂移（如中英文混用），防止图谱结构退化
