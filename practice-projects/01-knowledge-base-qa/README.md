...
| 编号 | 坑 | 现象 | 根因 | 方案 | 状态 |
|------|-----|------|------|------|------|
| A1 | 语义混淆 | 问硬件 Project X 答成营销 Project X | 向量检索余弦相似度极高 | 引入 Rerank + 调大 Top-K | ✅ |
| A3 | 切分策略 | 无法检索到长文档深处的密码 | 分块边缘截断 | 已使用 SentenceSplitter | ✅ |
| A4 | 质量评估 | 无法量化 RAG 表现 | 缺乏度量基准 | 已对接项目 00 评估流 | ✅ |

### 📅 2026-05-11 实验记录：从 Reranker 悖论到“宽进严出”
在引入 `BAAI/bge-reranker-base` 后，Case 1（语义混淆）依然失败的问题已解决。
- **原因**：初始检索（Top-K）设置太小（k=5），导致长文档的密集分块占满了候选位，营销类文档未能进入重排池。
- **方案**：在 `query_engine.py` 中将 `similarity_top_k` 提升至 **10**。
- **结论**：**Reranker 无法对它从未见过的文档进行排序。** 生产环境应采用“宽进（Top 10-20）严出（Top 2-5）”的二级过滤策略。
