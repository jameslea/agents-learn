...
from state import ErrorKind, FinalStatus
...
def main() -> None:
    ...
    passed = sum(1 for state in states if state.final_status == FinalStatus.PASSED)
    blocked = sum(1 for state in states if state.final_status == FinalStatus.BLOCKED)
    failed = sum(1 for state in states if state.final_status == FinalStatus.FAILED)
    safety_blocks = sum(1 for state in states for attempt in state.attempts if attempt.error.kind == ErrorKind.SECURITY_BLOCKED)
    timeouts = sum(
        1
        for state in states
    ...
    print(f"Effective success rate: {(passed + blocked) / len(states):.0%}")
    print(f"Repair success rate: {passed / len(states):.0%}")
    print(f"Average attempts for repaired tasks: {avg_attempts:.2f}")
    print(f"Timeout classifications: {timeouts}")
    print(f"Safety blocks: {safety_blocks + blocked}")
